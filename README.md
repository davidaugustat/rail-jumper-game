...
A current desktop or mobile browser with WebGL 2 and hardware acceleration is required. The portrait layout supports screens down to 320 × 568 pixels. Models and sounds are generated locally. Difficulty rises from 18 to 30 meters/second.

The endless world is a shared 3.2 km loop made from four 800 m districts: green outskirts, a rail yard, a long river crossing, and a dense city/tunnel corridor. Thirteen data-driven encounter families cover open barriers, mixed train rows, jump/slide chains, train weaves, multi-stage lane changes, ramps, roof hazards, passing-train transfers, multi-roof chains, roof exits, and reward sections. Every district can select every family but weights its thematic encounters more heavily, and the three most recently used families are excluded from selection.

Each generated encounter publishes timed lane, surface, jump, and duck steps. Before the encounter is committed, those normal inputs are replayed through the real fixed 120 Hz collision simulation alongside neighboring obstacles, including modest early and late timing variants. Unsafe candidates are rejected after bounded retries and replaced by a known-safe reward section. The opening 220 meters uses a gentler subset before multi-action and rooftop routes unlock. Coin singles, clusters, diagonals, jump arcs, ramp trails, roof rewards, and deliberate gaps vary pickup timing while retaining the original synthesized coin sound.

Obstacles are generated 650 meters ahead, while fog still fades from 220 to 560 meters. Instanced district scenery, merged procedural models, recycled encounter objects, and the looping world keep models and draw calls bounded.
...
The rooftop browser test renders ramp climbs, jumps to passing trains, all four districts, tunnels and the river bridges, and verifies the distant generation and model budget. `SCREENSHOTS=1 npm run test:rooftops` captures those scenes under `/tmp/railjumper-*.png`.
