...
export class World {
  scene = new T.Scene(); camera = new T.PerspectiveCamera(53, 1, .1, 850);
  ...
  constructor(canvas: HTMLCanvasElement) {
    ...
    this.renderer.setClearColor(0xbddeda); this.scene.fog = new T.Fog(0xbddeda, FOG_START, FOG_END);
    ...
    this.box(this.scene, [450, .4, 900], [0, -7, -420], 0x63adb4);
    this.box(this.scene, [10.5, .5, 850], [0, -.6, -400], colors.ballast);
    this.box(this.scene, [10.5, .2, 850], [0, -.25, -400], colors.ballast);
    for (const lane of [-3, 0, 3]) for (const dx of [-.75, .75]) this.box(this.scene, [.1, .14, 850], [lane + dx, -.08, -400], colors.rail);
    ...
    for (let i = 0; i < SCENERY_LENGTH / 2; i++) {
      ...
      if (!bridge && i % 5 === 0) {
        this.box(group, [200, .35, 10], [0, -.6, 0], colors.ground);
      ...
      if (!bridge && !tunnel && i % 15 === 0) {
        for (const side of [-1, 1]) this.box(group, [.2, 11.5, .2], [side * 5.8, 5.65, 0], colors.teal);
        this.box(group, [11.8, .22, .22], [0, 11.4, 0], colors.teal);
      }
  ...
  makeTunnel(base: number, length: number) {
    const group = new T.Group(); group.position.z = -base;
    for (const side of [-1, 1]) {
      this.box(group, [1.2, 8.8, length], [side * 6.1, 4.2, 0], 0x657d78);
      this.box(group, [.1, .25, length], [side * 5.46, 1.2, 0], 0xe4b967);
      for (let z = -length / 2; z <= length / 2; z += 10) {
        this.box(group, [.15, 7.3, .5], [side * 5.45, 3.65, z], 0x405c5b);
        this.box(group, [.18, .25, 1.8], [side * 5.4, 5.6, z], 0xffe4a0);
      }
    }
    const ceiling = new T.Group(); ceiling.name = 'ceiling'; group.add(ceiling);
    this.box(ceiling, [13.4, .6, length], [0, 8.6, 0], 0x526e69);
    for (const z of [-length / 2, length / 2]) {
      ...
      this.box(group, [3.6, .65, 1.25], [0, 8.15, z], 0x214e50);
      for (const x of [-.8,0,.8]) this.box(group, [.25,.25,1.3], [x,8.15,z], 0xffcb67);
    }
    this.scene.add(group); this.landmarks.push({ group, base, type: 'tunnel', length });
  }
  makeBridge(base: number, length: number) {
    const group = new T.Group(); group.position.z = -base;
    this.box(group, [12,.7,length], [0,-.8,0], 0x436665);
    for (const side of [-1,1]) {
      this.box(group, [.3,.3,length], [side * 5.7, 1.2,0], 0xda7952);
      this.box(group, [.4,.4,length], [side * 5.9, 7.7,0], 0x326b6b);
      for (let z = -length / 2; z <= length / 2; z += 11) {
        this.box(group, [.45, 15, .6], [side * 5.9,.2,z], 0x326b6b);
        const beam = this.box(group,[.26,10.8,.3],[side * 5.9,3.9,z + 5.5],0x438280); beam.rotation.x = .94;
        this.box(group,[12,.3,.35],[0,7.7,z],0x326b6b);
      }
    }
    this.scene.add(group); this.landmarks.push({ group, base, type: 'bridge', length });
  }
  ...
  makeEntity(e: Entity) {
    ...
    const g = this.pool.get(key)?.pop() ?? this.createEntity(e.kind, e.ramp); g.userData.kind = key; this.scene.add(g); return g;
  }
  createEntity(kind: string, ramp = false) {
    ...
    if (kind === 'coin') {
      ...
      const chassis = new T.Group(); chassis.name = 'chassis'; g.add(chassis);
      this.box(chassis, [2.35, 2.9, 7], [0, 1.65, 0], colors.teal);
      this.box(chassis, [2.45, .35, 7.05], [0, 3.1, 0], 0xe8e5c9);
      this.box(chassis, [2.37, .25, 7.04], [0, .75, 0], colors.coral);
      this.box(chassis, [1.8, .8, .04], [0, 2.3, 3.52], 0x173e4a);
      this.box(chassis, [.08, .85, .06], [0, 2.3, 3.55], 0xe1dcbf);
      for (const x of [-.8, .8]) this.box(chassis, [.3, .22, .07], [x, 1.1, 3.55], 0xffe6a0);
      for (const side of [-1, 1]) for (let z = -2.6; z < 3; z += 1.4) this.box(chassis, [.03, .85, .9], [side * 1.19, 2.25, z], 0x183f4a);
      for (const side of [-1, 1]) for (const z of [-2.3, 2.3]) this.box(chassis, [.2, .55, .7], [side * 1.05, .3, z], 0x284443);
      if (ramp) {
        const slope = new T.Group(); slope.name = 'ramp';
        const geometry = new T.BufferGeometry();
        const w = 1.16, h = ROOF_HEIGHT, r = RAMP_LENGTH;
        geometry.setAttribute('position', new T.Float32BufferAttribute([
          -w,0,r, w,0,r, w,h,0, -w,0,r, w,h,0, -w,h,0,
          -w,0,r, -w,h,0, -w,0,0, w,0,r, w,0,0, w,h,0,
          -w,0,0, -w,h,0, w,h,0, -w,0,0, w,h,0, w,0,0,
        ], 3));
        geometry.computeVertexNormals();
        const mesh = new T.Mesh(geometry, this.material(0xdfaa47)); mesh.castShadow = mesh.receiveShadow = true; slope.add(mesh);
        // High contrast chevrons show the ramp direction and rise.
        for (let d = 1; d < r; d += 2) {
          const stripe = this.box(slope, [1.9,.045,.32], [0, h * (1 - d / r) + .025,d], 0xffeeb5);
    ...
    if (kind === 'train') {
      ...
      const slope = g.getObjectByName('ramp'); if (slope) this.mergeModel(slope);
  ...
  mergeModel(root: T.Object3D) {
    ...
    root.traverse(child => {
      ...
      groups.get(material.uuid)!.geometries.push(child.geometry.clone().applyMatrix4(new T.Matrix4().multiplyMatrices(inverse, child.matrixWorld)));
    });
    root.clear();
    for (const { material, geometries } of groups.values()) {
      const merged = mergeGeometries(geometries)!;
      geometries.forEach(geometry => geometry.dispose());
      const mesh = new T.Mesh(merged, material); mesh.castShadow = mesh.receiveShadow = true; root.add(mesh);
    }
  }
  ...
  render(game: Game, time: number) {
    ...
    for (const batch of this.batches) {
      batch.matrices.forEach((matrix, index) => {
        ...
        matrix.elements[14] = ((originalZ + game.distance + 760) % SCENERY_LENGTH + SCENERY_LENGTH) % SCENERY_LENGTH - 760;
        batch.mesh.setMatrixAt(index, matrix); matrix.elements[14] = originalZ;
      });
      batch.mesh.instanceMatrix.needsUpdate = true;
    }
    for (const landmark of this.landmarks) {
      landmark.group.position.z = ((-landmark.base + game.distance + 690) % SCENERY_LENGTH + SCENERY_LENGTH) % SCENERY_LENGTH - 690;
    ...
    for (const e of game.entities) {
      ...
      if (mesh && mesh.userData.kind !== e.kind + (e.ramp ? '-ramp' : '')) { this.scene.remove(mesh); const key = mesh.userData.kind; if (!this.pool.has(key)) this.pool.set(key, []); this.pool.get(key)!.push(mesh); this.entityMeshes.delete(e.id); mesh = undefined; }
      ...
      if (e.kind === 'train') {
        ...
        const ramp = mesh.getObjectByName('ramp'); if (ramp) ramp.position.z = e.length / 2;
      }
    }
    ...
    this.camera.lookAt(game.x * .15, 1 + this.cameraHeight, -15);
